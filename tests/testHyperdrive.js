...
function create (key, opts) {
  if (!opts) opts = {}
  if(typeof opts.metadataKeyBook === 'undefined')
    opts = keybooks(opts)
  return hyperdrive(ram, key, opts)
}

function keybooks(opts){
    if (!opts) opts = {}
    opts.metadataKeyBook = new CryptoBook()
    opts.contentKeyBook = new CryptoBook()
    opts.metadataKeyBook.generateNewKey(0)
    opts.contentKeyBook.generateNewKey(0)
    return opts
}

// tests copied from hyperdrive repo...
...
module.exports = function () {
  ...
  tape('write and read (sparse)', function (t) {
    ...
    var opts = keybooks()
    var archive = create(opts)
    archive.on('ready', function () {
      opts.sparse = true
      var clone = create(archive.key, opts)
